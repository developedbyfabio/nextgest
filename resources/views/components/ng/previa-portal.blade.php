...
{{--
    ...
    cliente vê (x-portal.cabecalho / capa / como-funciona / servico) e, no LOGIN, o
    MESMO componente do login real (x-portal.auth) — sem maquete divergente. Num
    carrossel estilo Instagram dentro de uma moldura.

    Dois alternadores (só mudam a apresentação, nada de aparelho real):
    - claro/escuro: troca as superfícies da prévia (.ng-previa / .ng-previa.is-dark),
      independente do modo do painel ao redor.
    - celular/desktop: muda a LARGURA da moldura. Como o login é responsivo por
      container query, ele reflui sozinho (1 coluna no celular, 2 no desktop). As
      demais telas do portal centralizam em max-w-md (fiel ao portal real, que é
      mobile-first mesmo no desktop). O desktop é escalado para caber na coluna —
      a largura de layout (e o breakpoint) seguem reais; só o tamanho visual encolhe.
...
<div
    x-data="{
        ...
        tela: 0,
        total: 4,
        telas: ['Início', 'Login', 'Cliente', 'Agendar'],
        disp: 'mobile',
        escala: 1,
        ajustar() {
            if (this.disp !== 'desktop') { this.escala = 1; return; }
            const palco = this.$refs.palco;
            const largura = palco ? palco.clientWidth : 320;
            const rem = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
            this.escala = largura / (50 * rem);
        },
    }"
    x-init="$nextTick(() => ajustar()); $watch('disp', () => $nextTick(() => ajustar()))"
    @resize.window.debounce.150ms="ajustar()"
    class="flex flex-col items-center gap-3"
    wire:key="previa-portal"
>
    {{-- Alternadores: claro/escuro e celular/desktop — afetam SÓ a prévia. --}}
    <div class="flex flex-wrap items-center justify-center gap-2">
        ...
        <div class="inline-flex items-center gap-1 rounded-full border border-zinc-200 bg-white p-0.5 text-xs dark:border-zinc-700 dark:bg-zinc-800">
            <button type="button" @click="disp = 'mobile'"
                class="flex items-center gap-1 rounded-full px-2.5 py-1 font-medium transition"
                :class="disp === 'mobile' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-500'">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5"><path d="M8 1a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2H8Zm1.5 14.5a.75.75 0 0 1 .75-.75h.008a.75.75 0 0 1 0 1.5h-.008a.75.75 0 0 1-.75-.75Z"/></svg> Celular
            </button>
            <button type="button" @click="disp = 'desktop'"
                class="flex items-center gap-1 rounded-full px-2.5 py-1 font-medium transition"
                :class="disp === 'desktop' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-500'">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5"><path fill-rule="evenodd" d="M2 4.25A2.25 2.25 0 0 1 4.25 2h11.5A2.25 2.25 0 0 1 18 4.25v8.5A2.25 2.25 0 0 1 15.75 15h-3.105l.401 1.607.953.4a.75.75 0 0 1-.29 1.443H6.291a.75.75 0 0 1-.29-1.443l.953-.4L7.355 15H4.25A2.25 2.25 0 0 1 2 12.75v-8.5Zm1.5 0a.75.75 0 0 1 .75-.75h11.5a.75.75 0 0 1 .75.75v7.5a.75.75 0 0 1-.75.75H4.25a.75.75 0 0 1-.75-.75v-7.5Z" clip-rule="evenodd"/></svg> Desktop
            </button>
        </div>
    </div>

    {{-- Palco: largura da coluna. No desktop a moldura é escalada para caber (a
         largura de layout segue 50rem, então o login realmente reflui para 2
         colunas). overflow-hidden recorta o transbordo da caixa não-escalada. --}}
    <div class="relative w-full overflow-hidden" x-ref="palco"
        :style="{ height: disp === 'desktop' ? (escala * 30) + 'rem' : '40rem' }">

        <div class="origin-top-left transition-[transform,width] duration-300"
            :class="disp === 'desktop' ? '' : 'mx-auto max-w-full'"
            :style="disp === 'desktop' ? { width: '50rem', transform: 'scale(' + escala + ')' } : { width: '20rem' }">

            <div
                {{ $attributes->class('ng-previa relative w-full overflow-hidden shadow-2xl border-zinc-800 dark:border-zinc-700') }}
                :class="{ 'is-dark': dark, 'rounded-2xl border-[3px]': disp === 'desktop', 'rounded-[2.4rem] border-[7px]': disp !== 'desktop' }"
                :style="{ height: disp === 'desktop' ? '30rem' : '40rem' }"
            ...
            >
                ...
                <div class="flex h-full transition-transform duration-300 ease-out" :style="`transform: translateX(-${tela * 100}%)`">
                    ...
                    {{-- TELA 1 — Login do cliente (MESMO componente do login real) --}}
                    <div class="flex h-full w-full shrink-0 flex-col">
                        <x-portal.auth
                            :nome="$nome"
                            :logo-url="$logoUrl"
                            :fundo-url="$fundoUrl"
                            titulo="Entrar"
                            :subtitulo="'Acesse para agendar em ' . $nome"
                            class="min-h-0 flex-1 overflow-y-auto"
                        >
                            <div class="flex flex-col gap-4">
                                @foreach (['E-mail' => 'voce@exemplo.com', 'Senha' => '••••••••'] as $rotulo => $ph)
                                    <div class="flex flex-col gap-1">
                                        <span class="text-sm font-medium" style="color: var(--cor-texto);">{{ $rotulo }}</span>
    ...
    <flux:text class="text-center text-xs text-zinc-500">
        <span x-text="telas[tela]"></span> · claro/escuro e celular/desktop acima · atualiza enquanto você edita.
    </flux:text>
</div>
