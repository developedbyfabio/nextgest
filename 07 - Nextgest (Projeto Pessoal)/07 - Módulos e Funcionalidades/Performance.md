...
| **PERF-007** ✅ APLICADA | `vendas (cliente_id, status, data)` | índice composto (CREATE) | média | Agregação por cliente sobre comandas pagas (Indicadores Fase I). Já havia FK simples em `cliente_id`, mas faltava cobrir `status`+`data`. EXPLAIN (volume 8k): `index`/`Using where` (lookup por linha)→**`range`/`Using index` (covering)**, key `vendas_cliente_status_data_index`, linhas **7946→3973** | `ContagemQueriesTest` (Indicadores, constância) + [[Indicadores — motor (Fase I)]] |
